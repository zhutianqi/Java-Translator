...
public class Parser {
    ...
    private Boolean from_do_or_if_statement;
	...
	if ( is(TK.TILDE) ) {
		...
		if (table.search_index(table.size()-1, tok.string)) {
		    found = true;
		    translator.ref_id(search_expression, tok.string, table.size(), 0);
		}
		else {
	...
	if (search_expression == "") {
		...
		if (table.search_index(i, tok.string)) {
		    found = true;
		    translator.ref_id(search_expression, tok.string, table.size(), table.size() - i);
		    break;
		}
	    }
	}
